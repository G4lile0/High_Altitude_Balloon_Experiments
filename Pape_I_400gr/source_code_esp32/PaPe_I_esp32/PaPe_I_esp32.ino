...
//HardwareSerial gps(RX_GPS, TX_GPS);
...
// This is the default Semtech key, which is used by the early prototype TTN
// network.
static const u1_t PROGMEM APPSKEY[16] = { 0x00, };



// LoRaWAN end-device address (DevAddr) (formato MSB)
static const u4_t DEVADDR = 0x000000B ; // <-- Change this address for every node!
//**************************************
 ...
 * 
    ...
    if (EEPROM.read(dia*20) && (dia !=0)  && (altitude!=0) )  {
        int address = dia*20;
        for (byte n = 0; n < 10; n++) {
#ifdef DEBUG_ON
...
//  sensors.begin();
  ...
  gps.begin(9600, SERIAL_8N1, TX_GPS, RX_GPS);
  gps.setTimeout(2);
