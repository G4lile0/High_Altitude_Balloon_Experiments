/*******************************************************************************
   uPaPe_I
   by G4lile0  twitter: @G4lile0

   HW:  Arduino Pro mini (Atmel 328p 8Mhz 3.3V)

   Parte del código es grácias a:

    al ejemplo del envío de un paquete "Hello, world!" por LoRaWAN utilizando autenticación ABP, realizado por:
     Copyright (c) 2015 Thomas Telkamp and Matthijs Kooijman

    el GPS TTNMapper by Arirasan 2018 (Nodo LoRaWAN con GPS para realizar mapeo sobre TTNMapper)
      http://akirasan.net/nodo-de-mapeo-ttnmapper-arduino-gps-lorawan/
     twitter: @akirasan
...
//    Serial.println("SF");
 //   Serial.println(dr);
    ...
    if (n_packets>4) {
     ...
     if ((millis()-previousMillis)>8000) break;     // after 8 sec of not valid GPS break the loop and send temperature
    }
...
//      sats = nmea.getNumSatellites();
    ...
    // latitud, longitud
    // guardamos la primera posicion del dia si no esta escrita.
    // apaño porque si da el dia no da la altura... ??
     // uso la variable gps_set_sucess por reducir el uso de memoria


    if (nmea.getDay() !=0) { dia=nmea.getDay(); };
#ifdef DEBUG_ON
    Serial.println(F("day"));
    Serial.println(int(nmea.getDay()));
    Serial.println(EEPROM.read(int(nmea.getDay())*20) );
    Serial.println(F("dia"));
    Serial.println(dia);
#endif DEBUG_ON


     // guardamos el primer dato si no hay nada guardado, si tenemos dia, y la altitud no es cero
    if ((EEPROM.read(dia*20) == 255) && (dia !=0)  && (altitude!=0) )  {
        int address = dia*20;
        for (byte n = 0; n < 10; n++) {
#ifdef DEBUG_ON
           Serial.print(F("EEPROM WR: "));
           Serial.println(address+n,payload[n]);
#endif DEBUG_ON
           EEPROM.write (address+n,payload[n]);
        }
         };
...
//       Serial.println(sensors.getTempCByIndex(0));
       ...
       int batt = (int)(readVcc() / 25);  // readVCC returns  mVolt need just 100mVolt steps
       byte batvalue = (byte)batt; // no problem putting it into a int.

      payload[1] = (byte) batvalue;
 ...
 //       LMIC_setTxData2(2, payload, 1, 0);
...

       {
             ...
             // enviamos los datos del dia
             // guardamos el primer dato del dia si tenemos dia, y la altitud no es cero
             // miramos el primer dia a enviar..

#ifdef DEBUG_ON
                  Serial.print(F("Buscando eeprom dia "));
                  Serial.println(gps_set_sucess);
#endif DEBUG_ON



            if (EEPROM.read(gps_set_sucess*20)!=255 ) {

                int address = dia*20;
                for (byte n = 0; n < 10; n++) {
                  payload[n]=EEPROM.read (address+n);
                  ...
                  payload[9]=  gps_set_sucess;
                  //enviamos los datos recuperados de la memmoria eeprom
                  LMIC_setTxData2(3, payload, sizeof(payload) - 1, 0);
         };


          // subimos el dia de envio y si es mayor de 31 pasamos a cero
          gps_set_sucess++;
          if (gps_set_sucess==32) gps_set_sucess=1;
...
void setup() {
  ...
  // apagamos el MOSFET que controla la carga/led de las placas solares
    pinMode(A3, OUTPUT);
  digitalWrite(A3, LOW);
  delay(5000);
...
//  Serial.println("Initialising....");
  ...
  Serial.println(F("uBlox nav mode:"));
  uint8_t setNav[] = {
